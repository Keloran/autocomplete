...
const nerdctlGenerators: Record<string, Fig.Generator> = {
  runningContainerdContainers: {
    script: `nerdctl ps --format '{{ json . }}'`,
    postProcess: postProcessDockerPs,
  },
  allContainerdContainers: {
    script: `nerdctl ps -a --format '{{ json . }}'`,
    postProcess: postProcessDockerPs,
  },
  pausedContainerdContainers: {
    script: `nerdctl ps --filter status=paused --format '{{ json . }}'`,
    postProcess: postProcessDockerPs,
  },
  allLocalImages: {
    script: `nerdctl image ls --format '{{ json . }}'`,
  ...
  dockerHubSearch: {
    script: function (context) {
      ...
      return `nerdctl search ${searchTerm} --format '{{ json . }}'`;
    },
    postProcess: function (out) {
      return out
        .split("\n")
  ...
  allContainerdContexts: {
    script: `nerdctl context list --format '{{ json . }}'`,
    postProcess: function (out) {
      return out
        .split("\n")
        .map((line) => JSON.parse(line))
  ...
  listDockerPlugins: {
    script: `nerdctl plugin list --format '{{ json . }}'`,
    postProcess: sharedPostProcess,
  },
  listDockerSecrets: {
    script: `nerdctl secret list --format '{{ json . }}'`,
    postProcess: sharedPostProcess,
  },
  listDockerServices: {
    script: `nerdctl service list --format '{{ json . }}'`,
    postProcess: function (out) {
      return out
        .split("\n")
        .map((line) => JSON.parse(line))
  ...
  listDockerStacks: {
    script: `nerdctl stack list --format '{{ json . }}'`,
    postProcess: function (out) {
      return out
        .split("\n")
        .map((line) => JSON.parse(line))
  ...
  listDockerVolumes: {
    script: `nerdctl volume list --format '{{ json . }}'`,
    postProcess: function (out) {
      return out
        .split("\n")
        .map((line) => JSON.parse(line))
...
const containersArg = {
  name: "container",
  generators: nerdctlGenerators.runningContainerdContainers,
};

const imagesArg = {
  name: "image",
  generators: nerdctlGenerators.allLocalImages,
};
...
const contextsArg = {
  name: "CONTEXT",
  generators: nerdctlGenerators.allContainerdContexts,
};
...
const sharedCommands: Record<string, Fig.Subcommand> = {
  ...
  rm: {
    ...
    args: {
      isVariadic: true,
      name: "containers",
      generators: nerdctlGenerators.allContainerdContainers,
    },
    options: [
      {
        name: ["-f", "--force"],
...
default-cgroupns-mode option on the daemon (default)`,
    ...
    args: [
      {
        ...
        generators: {
          script:
            "nerdctl images --format '{{.Repository}} {{.Size}} {{.Tag}} {{.ID}}'",
          postProcess: function (out) {
            return out.split("\n").map((image) => {
  ...
  unpause: {
    ...
    args: {
      name: "container",
      generators: nerdctlGenerators.pausedContainerdContainers,
    },
  },
  update: {
    name: "update",
  ...
  pull: {
    ...
    args: {
      ...
      generators: nerdctlGenerators.dockerHubSearch,
      debounce: true,
    },
    options: [
      {
    ...
    },
    {
      ...
      args: {
        ...
        generators: [
          ...
          {
            script: `nerdctl images -a --format '{{ json . }}'`,
            postProcess: function (out) {
              const allLines = out.split("\n").map((line) => JSON.parse(line));
              return allLines.map((i) => {
                let displayName;
          ...
          {
            script: `nerdctl volume ls --format '{{ json . }}'`,
            postProcess: function (out) {
              const allLines = out.split("\n").map((line) => JSON.parse(line));
              return allLines.map((i) => ({
                name: i.Name,
      ...
      subcommands: [
        {
          ...
          args: [
            {
              name: "NETWORK",
              generators: nerdctlGenerators.listContainerdNetworks,
            },
            containersArg,
          ],
          options: [
      ...
      subcommands: [
        {
          ...
          args: {
            name: "NODE",
            generators: nerdctlGenerators.listDockerSwarmNodes,
            isVariadic: true,
          },
        },
        {
    ...
    },
    {
      ...
      subcommands: [
        ...
        },
        {
          ...
          args: {
            name: "PLUGIN",
            generators: nerdctlGenerators.listDockerPlugins,
          },
          options: [
            {
              name: ["-f", "--force"],
